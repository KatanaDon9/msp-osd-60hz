...
static uint8_t frame_ready = 0;  // SPEEDUP: Flag for immediate send
uint8_t update_rate_hz = 60;     // SPEEDUP: Higher default (was 2)
...
static void msp_draw_complete() {
    ...
    frame_ready = 1;  // SPEEDUP: Signal frame ready for immediate send
}
...
int main(int argc, char *argv[]) {
    ...
    while (!quit) {
        ...
        poll(poll_fds, 2, 1);  // SPEEDUP: 1ms timeout (was 50ms)
        ...
        // SPEEDUP: Send immediately when frame ready (was timer-based)
        if(compress && frame_ready) {
